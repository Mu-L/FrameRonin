...
/** 在图片下方追加全透明区域（原图画在顶部，不改变原图尺寸内的像素） */
export async function appendTransparentBottom(blob: Blob, bottomPx: number): Promise<Blob> {
  if (bottomPx <= 0) return blob
  const img = await (typeof createImageBitmap === 'function'
    ? createImageBitmap(blob)
    : new Promise<HTMLImageElement>((resolve, reject) => {
        const im = new Image()
...
/**
 * RPGMAKER V2：期望输入 192×320（上 256 高区内为 4 行×64 角色表，下 64 为透明预留）。
 * 第 3、4 行（1-based）整体下移 64px；原第三行位置填入第二行内容的水平镜像。
 */
export async function applyRpgMakerV2FourRowLayout(blob: Blob): Promise<Blob> {
  const ROW = 64
  const EXPECT_W = 192
  const EXPECT_H = 320
  const img = await (typeof createImageBitmap === 'function'
  ...
  if (img.width !== EXPECT_W || img.height !== EXPECT_H) {
    throw new Error(`RPGMAKER V2 row layout expects ${EXPECT_W}×${EXPECT_H}, got ${img.width}×${img.height}`)
  }
  const w = EXPECT_W
  const canvas = document.createElement('canvas')
  canvas.width = w
  canvas.height = EXPECT_H
  const ctx = canvas.getContext('2d')
  if (!ctx) throw new Error('ERR_CANVAS_CREATE')
  ctx.clearRect(0, 0, w, EXPECT_H)
  ctx.imageSmoothingEnabled = false
  ctx.imageSmoothingQuality = 'low'
  ctx.drawImage(img, 0, 0, w, ROW, 0, 0, w, ROW)
  ctx.drawImage(img, 0, ROW, w, ROW, 0, ROW, w, ROW)
  ctx.save()
  ctx.translate(w, 2 * ROW)
  ctx.scale(-1, 1)
  ctx.drawImage(img, 0, ROW, w, ROW, 0, 0, w, ROW)
  ctx.restore()
  ctx.drawImage(img, 0, 2 * ROW, w, ROW, 0, 3 * ROW, w, ROW)
  ctx.drawImage(img, 0, 3 * ROW, w, ROW, 0, 4 * ROW, w, ROW)
  return new Promise((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error('ERR_TOBLOB'))), 'image/png', 0.95)
  })
}
...
/**
 * 将图均分为 rows×cols 格，每格四边各裁 trimPx 像素后按原顺序拼回。
 * 宽高必须能整除；每格裁后尺寸为 (cellW-2*trimPx)×(cellH-2*trimPx)。
 */
export async function trimGridCellsEdgeAndMerge(
  blob: Blob,
  opts: { rows: number; cols: number; trimPx: number }
): Promise<Blob> {
  const { rows, cols, trimPx } = opts
  if (rows < 1 || cols < 1 || trimPx < 0) throw new Error('ERR_TRIM_GRID_PARAMS')
  const img = await (typeof createImageBitmap === 'function'
    ? createImageBitmap(blob)
    : new Promise<HTMLImageElement>((resolve, reject) => {
        const im = new Image()
  ...
  if (w % cols !== 0 || h % rows !== 0) {
    throw new Error(`trim grid: size ${w}×${h} not divisible by ${cols}×${rows}`)
  }
  const cellW = w / cols
  const cellH = h / rows
  if (trimPx * 2 >= cellW || trimPx * 2 >= cellH) {
    throw new Error(`trim ${trimPx}px too large for cell ${cellW}×${cellH}`)
  }
  const innerW = cellW - 2 * trimPx
  const innerH = cellH - 2 * trimPx
  const outW = cols * innerW
  const outH = rows * innerH
  const canvas = document.createElement('canvas')
  canvas.width = outW
  canvas.height = outH
  const ctx = canvas.getContext('2d')
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const sx = c * cellW + trimPx
      const sy = r * cellH + trimPx
      const dx = c * innerW
      const dy = r * innerH
      ctx.drawImage(img, sx, sy, innerW, innerH, dx, dy, innerW, innerH)
    }
  }
  return new Promise((resolve, reject) => {
    canvas.toBlob((b) => (b ? resolve(b) : reject(new Error('ERR_TOBLOB'))), 'image/png', 0.95)
